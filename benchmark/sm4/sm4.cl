#define opencl_memcpy(dst, src, count)					\
...
__constant const unsigned int SM4_CK[32] = {
...
__constant const unsigned char SM4_Sbox[256] = {
...
__constant const unsigned int SM4_FK[4] = {0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC};

static void SM4_KeySchedule(__global unsigned char MK[],unsigned int rk[])
{
...
//-------------------------------------------------------------------------------------------------
static void SM4_Encrypt(__global unsigned char MK[],__global unsigned char PlainText[],__global unsigned char CipherText[])
{
...
static void SM4_Decrypt(__global unsigned char MK[],__global unsigned char CipherText[],__global unsigned char PlainText[])
{
    unsigned int rk[32],X[36],tmp,buf;
    int i,j;
    SM4_KeySchedule(MK,rk);
    ...
    __global unsigned char *result)
{
    ...
    SM4_Encrypt(key,input+idx*16,result+idx*16);
    //opencl_memcpy(result,key,16);
    return ;
}

__kernel void decrypt_kernel(
    __global const unsigned char *key,
    __global const unsigned char *input,
    __global unsigned char *result)
{
    ...
    SM4_Decrypt(key,input+idx*16,result+idx*16);
    //opencl_memcpy(result,input,16);
    return ;
}
